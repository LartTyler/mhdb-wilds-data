use crate::placeholders_old::enemy::Enemy;
use crate::placeholders_old::listener::Listener;
use crate::placeholders_old::reference::Reference;
    ...
    pub fn extract(value: &str) -> Vec<Self> {
        ...
        for item in Item::extract(value) {
            let placeholder = match item.kind() {
                ...
                "EMID" => Self::Enemy(Enemy::normal(item.value)),
                "EMIDJP" => Self::Enemy(Enemy::kanji(item.value)),
                "BOLD" | "/BOLD" | "COLOR" | "/COLOR" => Self::Remove(item.value),
                v => panic!("Unrecognized placeholder name '{v}' in '{}'", value),
            };
...
impl ApplyPlaceholder for Placeholder {
    fn apply(&self, value: &str, context: &ApplyContext<'_>) -> String {
        match self {
            ...
            Self::Enemy(v) => v.apply(value, context),
